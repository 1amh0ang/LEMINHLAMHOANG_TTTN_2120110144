...
namespace VietnamWatches.Areas.Admin.Controllers
{
    public class UserController : BaseController
    {
        ...
        public ActionResult DelTrash(int? userId)
        {
            if (userId == null)
            {
                TempData["message"] = new XMessage("danger", "Mã loại User không tồn tại!");
                return RedirectToAction("Index", "User");
            }
            User user = userDAO.getRow(userId);
            if (user == null)
            ...
            user.Status = 0; //Chuyen vao thung rac
        ...
        public ActionResult ReTrash(int? id)
        {
            ...
            user.Status = 2; //Khôi phục trạng thái
            user.UpdatedBy = Convert.ToInt32(Session["UserId"].ToString());
            user.UpdatedAt = DateTime.Now;
            userDAO.Update(user);
